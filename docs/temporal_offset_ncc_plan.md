...
`data/Odom.jsonl`과 `data/pose_GT_by_mocap.jsonl` 사이의 temporal offset을 normalized cross correlation, NCC로 추정한다. 두 JSONL은 pose가 아니라 `vx`, `vy`, `vtheta` velocity를 담는다. 추정된 offset을 이용해 두 trajectory를 같은 시간축에 정렬한 뒤, 이후 odom-to-GT mapping 모델 학습에 사용할 수 있는 보정 JSONL을 `data` 디렉토리에 저장한다.
...
`vx`, `vy`는 각 파일의 첫 pose frame에서 계산된 translation velocity이고, `vtheta`는 yaw rate이다.
...
따라서 NCC는 pose 자체가 아니라 velocity feature에서 수행한다.

기본 feature:

- linear speed: `speed = sqrt(vx^2 + vy^2)`
- yaw rate: `omega = vtheta`
...
1. Load JSONL
   ...
   - `timestamp`, `vx`, `vy`, `vtheta`를 float array로 읽는다.
   - timestamp 순서로 정렬한다.
...
3. Interpolate Velocity

   - uniform grid에 `vx`, `vy`, `vtheta`를 선형 보간한다.
...
6. Search Lag
   ...
   - 각 lag에서 짧은 sliding window를 쓰지 않는다.
   - 각 lag에서 두 feature signal이 겹치는 전체 overlap interval을 사용한다.
   - overlap sample 수가 너무 적으면 해당 lag는 버린다.
   - score는 overlap 구간의 normalized dot product로 계산한다.
   - 최종 offset은 이 lag range 안에서 전체 overlap NCC score가 가장 높은 global constant lag로 선택한다.
...
8. Generate Temporally Aligned JSONL
   ...
   - 두 velocity trajectory를 같은 relative timestamp grid로 resampling한다.
   - 출력 timestamp는 `0.0`부터 시작하는 aligned relative time으로 둔다.
   - 출력 row key는 `timestamp`, `vx`, `vy`, `vtheta`만 유지한다.
