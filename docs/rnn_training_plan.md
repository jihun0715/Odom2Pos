...
레포 최상단의 `config.py`에서 주요 학습 파라미터를 관리한다. 이미 만들어져 있는 top-level `config.py`를 사용하고, `src/config.py`는 만들지 않는다.

`scripts/run_train_rnn.sh`는 레포 root에서 `src/train_rnn.py`를 실행하므로, `train_rnn.py`는 top-level `config.py`를 import한다. 시작 기본값은 다음처럼 둔다.
...
- top-level `config.py` 값 변경만으로 `hidden_size`, `num_layers`, `seq_len`, `learning_rate`, `loss_type`을 바꿀 수 있다.
