...
Temporal alignment가 끝난 velocity JSONL을 사용해 odom-to-GT RNN 모델을 학습하려면 다음 스크립트를 실행합니다.

```bash
bash scripts/run_train_rnn.sh
```

학습 설정은 레포 최상단 `config.py`에서 관리합니다. 기본값은 CPU 학습, 2-layer RNN, hidden size 32, Adam optimizer, BerHu loss입니다.
